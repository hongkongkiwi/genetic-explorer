...
export const APIRoute = createAPIFileRoute('/api/users/export')({
  POST: async ({ request }) => {
    try {
      ...
      // Validate request
      if (!password) {
        return json({ success: false, error: 'Password is required for verification' }, { status: 400 });
      }
      ...
      if (is2faRequiredForAction('export')) {
        ...
        if (twoFactor.method === 'totp') {
          ...
          return json({ success: false, error: 'TOTP verification not yet configured' }, { status: 501 });
        ...
        if (!is2faValid) {
          logActivity(auth.id, 'export_failed', 'export', auth.id, { reason: 'invalid_2fa', method: twoFactor.method }, ipAddress);
      ...
      logActivity(auth.id, 'export_verified', 'export', auth.id, { format, include }, ipAddress);
  ...
  GET: async ({ request }) => {
    try {
      ...
      const format = (url.searchParams.get('format') as 'json' | 'zip') || 'json';
      ...
      if (!verificationKey || !isExportVerified(verificationKey, auth.id)) {
        return json({ success: false, error: 'Export verification required. Please verify your identity first.' }, { status: 403 });
      }
      ...
      // Log export
      logActivity(auth.id, 'data_exported', 'export', auth.id, {
        format,
        includeProfile,
        includeGenomes,
        includeReports,
      ...
      if (format === 'json') {
        ...
        return json({ success: false, error: 'ZIP format not yet implemented' }, { status: 501 });
      }
    } catch (error) {
      if (error instanceof Error && error.message === 'Unauthorized') {
        return json({ success: false, error: 'Unauthorized' }, { status: 401 });
...
export const APIRouteSendVerification = createAPIFileRoute('/api/users/export/send-verification')({
  POST: async ({ request }) => {
    try {
      const auth = requireAuth(request);

      // Generate and store email code
      const { generateEmailCode, storeEmailCode } = require('~/utils/twoFactor');
      ...
      // Log the request
      const ipAddress = getClientIp(request);
      logActivity(auth.id, 'export_verification_sent', 'export', auth.id, {}, ipAddress);
