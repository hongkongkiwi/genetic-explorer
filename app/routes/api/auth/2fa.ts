...
export const APIRouteSetup = createAPIFileRoute('/api/auth/2fa')({
  POST: async ({ request }) => {
    try {
      ...
      if (method === 'passkey') {
        ...
        const existingCredentials = getPasskeys(auth.id).map(
          (p: any) => p.credentialId,
        )
...
export const APIRouteVerify = createAPIFileRoute('/api/auth/2fa')({
  POST: async ({ request }) => {
    try {
      const auth = requireAuth(request)
      const ipAddress = getClientIp(request)
      ...
      if (method === 'totp') {
        ...
        if (isValid) {
          ...
          const hashedCodes = backupCodes.map((code: string) =>
            hashBackupCode(code, auth.id),
          )
        ...
        if (verificationResult.valid) {
          ...
          verificationData = {
            ...
            message:
              'Passkey registered successfully! Save these backup codes in a safe place.',
          }
        } else {
          isValid = false
        }
        ...
        if (isValid) {
          ...
          verificationData = {
            message:
              '2FA enabled successfully! Use an authenticator app for future logins.',
          }
        }
...
export const APIRouteStatus = createAPIFileRoute('/api/auth/2fa')({
  GET: async ({ request }) => {
    try {
      const auth = requireAuth(request)
...
export const APIRouteDisable = createAPIFileRoute('/api/auth/2fa')({
  POST: async ({ request }) => {
    try {
      const auth = requireAuth(request)
      const ipAddress = getClientIp(request)
...
export const APIRouteRegenerateBackupCodes = createAPIFileRoute(
  '/api/auth/2fa',
)({
  POST: async ({ request }) => {
    try {
...
export const APIRoutePasskeyAuth = createAPIFileRoute('/api/auth/2fa')({
  GET: async ({ request }) => {
    try {
      const auth = requireAuth(request)
