...
#### M1: Console Error Messages May Leak Information
...
**Mitigation:** Console messages are generic; detailed errors are not exposed to users.  
**Recommendation:** Ensure production builds strip debug console statements.

#### M2: Email in Plaintext
**Location:** `users` table  
**Issue:** Email addresses stored in plaintext (required for functionality).  
**Mitigation:** Database is encrypted at rest; access controls in place.  
**Recommendation:** Consider deterministic encryption for emails if regulatory requirements demand it.

### Low Risks: 2

#### L1: PDF Export Uses innerHTML
**Location:** `app/utils/pdfExport.ts`  
**Issue:** `container.innerHTML = html` for PDF generation.  
**Mitigation:** HTML content is controlled/generated by the application, not user input.  
**Recommendation:** Validate HTML content before insertion.

#### L2: Test Files Use innerHTML
**Location:** Test files only  
**Issue:** Test code accesses `innerHTML` for assertions.  
**Mitigation:** Test-only code, not production.  
